...
public class ArrayHeapMinPq {
    ...
    public void add(int item, int weight) {
        ...
        if (lastEmptySpot * 4 / 3 >= arrayHeap.length - 1) {
            extend();
        }
    }
    ...
    private void sinkItem(int item, int index) {
        ...
        if (smallChildIndex == -1) {
            return;
        }
        int smallChild = arrayHeap[smallChildIndex][1];
    ...
    private void floatItem(int item, int index) {
        if (index >= 1) {
            ...
            int parent = arrayHeap[parentIndex][1];
    ...
    public int getSmallestChildIndex(int index) {
        if (index >= lastEmptySpot) {
            return -1;
        }
        int lC = leftChild(index);
        int rC = rightChild(index);
        if (arrayHeap[lC][1] > arrayHeap[rC][1]) {
            return rC;
        }
        return lC;
    }
